                ...
                $.each(plot.getAxes(), function(axisName, axis) {
                    ...
                    if (opts.axisLabelUseCanvas) {
                        ...
                        if (axisName.charAt(0) == 'x') {
							...
							//doing without aling it to the center
                            x = plot.getCanvas().width - width;
							if(opts.centralize)
								x= plot.getPlotOffset().left + plot.width()/2 - width/2;
                            ...
                            y = plot.getCanvas().height;
 							...
 							//better formula that allows a large label without part of it been eaten
 							y = plot.getPlotOffset().top +  width;
 							//if is centralized will add more width to y
 							if(opts.centralize)
